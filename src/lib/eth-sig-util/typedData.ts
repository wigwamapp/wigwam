...
import { rawEncode, soliditySHA3 } from "./abi";
import {
  concatSig,
  isNullish,
  legacyToBuffer,
  recoverPublicKey,
  legacyKeccak as keccak,
} from "./utils";
...
function encodeField(
  ...
  if (type.lastIndexOf("]") === type.length - 1) {
    ...
    const typeValuePairs: [type: string, value: any][] = (value as any[]).map(
...
function findTypeDependencies(
  ...
  const matched = primaryType.match(/^\w*/u);
...
function hashType(
  ...
  return keccak(toUtf8Bytes(encodeType(primaryType, types)));
}
...
function sanitizeData<T extends MessageTypes>(
  ...
  if ("types" in sanitizedData) {
    (sanitizedData as any).types = { EIP712Domain: [], ...sanitizedData.types };
  }
